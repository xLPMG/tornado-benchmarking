...
public class Compute {
    ...
    private static void mxmLoopSequential(Matrix2DFloat A, Matrix2DFloat B, Matrix2DFloat C, final int size) {
    ...
    public void run(Matrix2DFloat A, Matrix2DFloat B, Matrix2DFloat C, final int size, boolean parallelize) {
        ...
        // Create a task-graph with multiple tasks. Each task points to an exising Java
        // method
        // that can be accelerated on a GPU/FPGA
        TaskGraph taskGraph = new TaskGraph("myCompute")
                .transferToDevice(DataTransferMode.FIRST_EXECUTION, A, B) // Transfer data from host to device only in
                                                                          // the first execution
                .task("mxm", parallelize ? Compute::mxmLoopParallel : Compute::mxmLoopSequential, A, B, C, size) // Each
                                                                                                                 // task
                                                                                                                 // points
                                                                                                                 // to
                                                                                                                 // an
